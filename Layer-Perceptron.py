...
class allfunction():
    def __init__(self, filename):
        ...
        self.color_dict={0:'blue',1:'red',2:'yellow',3:'green'}
        self.label=1                            #標籤種類
    ...
    #---讀檔、基礎設定、畫圖、
    def readfile(self): 
        with open(os.getcwd()+'/NN_HW1_DataSet/'+ self.filename +".txt", 'r', encoding='UTF-8') as file:
    ...
    def basic_setting(self):
        if(self.dimension>3):
            messagebox.showwarning("Warning","四維以上無法顯示")
        else:
    ...
    def check_learning_setting(self):
        if(data_combobox.current()==-1): 
            messagebox.showwarning("Warning","請選擇訓練資料")
            return False
        
        if(lr.get()==""):
            messagebox.showwarning("Warning","請輸入學習率")
            return False
        else:
            try:
                ...
                messagebox.showwarning("Warning","學習率請輸入整數或浮點數")
                return False
                
        if(convergence.get()==0):
            if(epoch_accuracy.get()==""):
                messagebox.showwarning("Warning","請輸入希望訓練次數")
            else:
                try:
                    self.epoch=int(epoch_accuracy.get())
                    return True
                except:
                    messagebox.showwarning("Warning","訓練次數請輸入整數")
                    return False
        else:
            if(epoch_accuracy.get()==""):
                messagebox.showwarning("Warning","請輸入希望準確率")
            else:
                try:
                    self.accuracy = float(epoch_accuracy.get())
                    return True
                except:
                    messagebox.showwarning("Warning","準確率請輸入整數或浮點數")
                    return False
                return
...
window.title("類神經網路_感知機訓練器PLA")

#背景
def create_label_image():
    imgpath='C:\\Users\\USER\\Downloads\\perceptron-main\\true.gif'
    image = Image.open(imgpath) 
    img = image.resize( (image.width * 3, image.height *1) )
    im = ImageTk.PhotoImage(img)
    lbl_2 = tk.Label(window, image=im)
    lbl_2.image = im
    lbl_2.grid(column=0, row=0)
#create_label_image()

file=["2Ccircle1", "2Circle1", "2Circle2", "2CloseS", "2CloseS2", "2CloseS3", "2cring",\
      "2CS","2Hcircle1", "2ring", "4satellite-6", "5CloseS1", "8OX", "C3D", "C10D",\
      "IRIS", "Number",  "perceptron1", "perceptron2", "perceptron3", "perceptron4",\
      "wine", "xor", "test_4_2"]

#設定框_1"
setting1 = tk.Frame(window)
setting1.grid(row=0, column=0, sticky=tk.N, padx=15)
...
tk.Entry(setting1, width=10, textvariable=lr).grid(row=0, sticky=tk.N, padx=80, pady=15)


#設定框_1_收斂條件
convergence = tk.IntVar() #判斷收斂條件
epoch_accuracy = tk.StringVar() #訓練次數、正確率
epoch_accuracy.set("1")
convergence_condition_text = tk.Label(setting1, font=("微軟正黑體", 12, "bold"), text="收斂條件：").grid(row=2, sticky=tk.W, pady=2)
tk.Radiobutton(setting1, font=("微軟正黑體", 10, "bold"), text="訓練次數 (整數表達)", variable=convergence, value=0).grid(row=3, sticky=tk.W, padx=20)
tk.Radiobutton(setting1, font=("微軟正黑體", 10, "bold"), text="測試資料準確率 (小數點表達)", variable=convergence, value=1).grid(row=4, sticky=tk.W, padx=20)
tk.Entry(setting1, width=10, textvariable=epoch_accuracy).grid(row=2, sticky=tk.N, pady=15)
#----------------------------------------------------------------

#設定框_2"
setting2 = tk.Frame(window)
setting2.grid(row=0, column=2, sticky=tk.N, padx=15)

#設定框_2_視覺化更新速度：
tk.Label(setting2, font=("微軟正黑體", 12, "bold"), text="選擇速度：").grid(row=0, sticky=tk.W, pady=4)
update_combobox = ttk.Combobox(setting2, width=10, value=['慢速','正常','快速'], state="readonly") #readonly為只可讀狀態
update_combobox.grid(row=0, column=0, sticky=tk.W, padx=117, pady=15)
update_combobox.current(1) #預設Combobox為index0


#設定框_2_訓練資料集
tk.Label(setting2, font=("微軟正黑體", 12, "bold"), text="選擇訓練資料：").grid(row=2, sticky=tk.W, pady=4)
data_combobox = ttk.Combobox(setting2, width=10, value=file, state="readonly") #readonly為只可讀狀態
data_combobox.grid(row=2, column=0, sticky=tk.W, padx=117, pady=15)
data_combobox.bind("<<ComboboxSelected>>", creatobject)

#設定框_2_開始訓練按鈕
btn = tk.Button(setting2, width=10, bg='pink', text='開始訓練')
btn.grid(row=3, sticky=tk.W, padx=180, pady=20)

def stop():
    os.system('pause')
btn4 = tk.Button(setting2, width=10, bg='gray', text='暫停訓練', command=stop)
btn4.grid(row=3, sticky=tk.W, padx=80, pady=20)


#----------------------------------------------------------------
tk.Label(window, font=("微軟正黑體", 8, "bold"), text=" 2/3 訓練；1/3 測試").grid(row=4, pady=4, sticky=tk.N)
btn1 = tk.Button(window, bg='PowderBlue', text='全部資料 (All)')
btn1.grid(row=4, column=0, pady=50, sticky=tk.N)
btn2 = tk.Button(window, bg='Azure', text='訓練資料 (Train)')
btn2.grid(row=4, column=0, pady=140, sticky=tk.N)
btn3 = tk.Button(window, bg='LightCyan', text='測試資料 (Test)')
btn3.grid(row=4, column=0, pady=90, sticky=tk.N)
#訓練圖
training_plot = tk.Frame(window, padx=5,bg='LightCyan')
training_plot.grid(row=4, column=0,columnspan=4, padx=200, sticky=tk.N)
...
result_1.grid(row=5, column=0, columnspan=2, sticky=tk.NW, padx=15)

lr_display = tk.StringVar()
#tk.Label(result_1, font=("微軟正黑體", 10, "bold"), text="學習率： ").grid(row=0, column=0, sticky=tk.W)
...
tk.Label(result_1, font=("微軟正黑體", 10, "bold"), text="鍵結值向量： ").grid(row=4, column=0, sticky=tk.W)
...
result_2.grid(row=5, column=2, columnspan=2, sticky=tk.NW)
dataset = tk.StringVar()#資料集
#tk.Label(result_2, font=("微軟正黑體", 10, "bold"), text="訓練資料： ").grid(row=0, column=0, sticky=tk.W)
...
tk.Label(result_2, font=("微軟正黑體", 10, "bold"), text="訓練進度： ").grid(row=1, column=0, sticky=tk.W)
...
tk.Label(result_2, font=("微軟正黑體", 10, "bold"), text="訓練正確率： ").grid(row=2, column=0, sticky=tk.W)
...
tk.Label(result_2, font=("微軟正黑體", 10, "bold"), text="測試正確率： ").grid(row=3, column=0, sticky=tk.W)
